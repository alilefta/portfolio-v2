...
import { getLabNotes, LabNote } from "@/lib/notes";

const TypeIcon = ({ type }: { type: LabNote["metadata"]["type"] }) => {
  switch (type) {
    case "Snippet":
      return (
...
export default function LabNotesScroll() {
  const labNotes = getLabNotes().slice(0, 5);
  return (
    <section className="mx-auto w-full max-w-7xl border-t border-zinc-200 px-4 py-12 md:px-6 dark:border-zinc-800">
      ...
      <div className="group/container relative">
        ...
        <div className="pointer-events-none absolute top-0 bottom-0 left-0 z-10 w-8 bg-linear-to-r from-white to-transparent md:hidden dark:from-black" />
        <div className="pointer-events-none absolute top-0 right-0 bottom-0 z-10 w-8 bg-linear-to-l from-white to-transparent md:hidden dark:from-black" />
          ...
          {labNotes.map((note) => (
            ...
            >
              ...
              <div className="flex items-start justify-between">
                ...
                >
                  <TypeIcon type={note.metadata.type} />
                </div>
              </div>
